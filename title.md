...
- 引发 LTD：$0.2 \, \text{umol}$。
- 引发 LTP：$0.8 \, \text{umol}$。
- 单次发放进入：$0.25 \, \text{umol}$。
- 因果进入（见下文树突结构），令 $h$ 为 NMDA 开放率：$ 2 \cdot h \, \text{umol}$。
...
f(x) = \begin{cases}
0 & x \le 0.2 \\
\large\frac{0.5\, \cdot \,(x \,- \,0.2) \,\cdot \,(x \,- \,0.8)}{ x ^ 2 \,+\, 0.3}  & x > 0.2
...
![](https://cdn.luogu.com.cn/upload/image_hosting/tsozsvnp.png)
...
- 突触后神经元发放，给予了去极化，但是没有谷氨酸结合，NMDA 依然无法打开，只会通过 VGCCs 进入较少。

- 既有前神经元的发放，也有后神经元的发放，两个锁都打开，$\text{Ca}^{2+}$ 可以通过 NMDA 进入。
...
请注意请注意请注意，下文的 $\text{L}$ 一串和平常说的视觉皮层哪些不是一个东西。
...
- $\text{L}_\text{1}$ 很有趣，它几乎没有具体的神经元，大部分是同区 $\text{L}_\text{2/3}$ 的顶树突构成。

- $\text{L}_\text{6}$ 是管理员，这家伙把轴突全插到丘脑中了，告诉丘脑现在我要多大强度的信息。

- $\text{L}_5$：输出，神经元大，拥有巨大的顶树突，一直向上到 $\text{L}_1$，轴突大多伸出，那么输出什么呢。发放预测与运动指令，对于运动，她的树突很独特：
...
- $\text{L}_{2/3}$：整合，接收 $\text{L}_4$ 信息，投射到 $\text{L}_5$。
- $\text{L}_\text{2/3}\to \text{L}_\text{2/3}$  横向链接
- $\text{L}_\text{2/3}\to \text{L}_\text{5}$ 处理好信息了，让我们干活


对于高区到低区
- $\text{L}_\text{2/3}\to \text{L}_\text{1}$ 提供一些上游语境之类的。
- $\text{L}_\text{5}\to \text{L}_\text{1}$
- $\text{L}_4$：输入，$\text{L}_{2/3}$。
- $\text{L}_6$：向上传输反馈信号。
- $\text{L}_2$ 主要负责横向，$\text{L}_3$ 主要负责跨层。
- 高级 $\text{L}_{2/3}$ 报告误差。
  

对于低区到高区。

- $\text{L}_\text{2/3}\to \text{L}_\text{4}$，  比如视觉 V1 到 V2/4  的上传。
...
PFC 的 $\text{L}_\text{1...6}$ 发育的都很好，都很好！
...
其中：PN 代表主椎体神经元，VIP 和 SST 都是抑制性神经元。
